...
| 零 Mock 動態原則 | 嚴禁在 RAG 核心或 TUI 當中塞入任何硬編碼的靜態 Mock 文件（如私人 IP、區網機器、或硬寫死 mock 檔案）。TUI 與 RAG 必須完全、且動態對接 SQLite 實體資料庫，實時查詢、過濾與反映當前工作空間中的實體檔案，做到真實呈現。 |
