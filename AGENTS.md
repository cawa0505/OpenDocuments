...
> 2. [ ] 編譯與安裝已驗證 (`cargo check` 零錯誤 → `cargo install --path . --force`)
...
## 2. 工作區修正协议 (要求協議)
> 以下是 2026-07-xx 期间修正的核心问题，避免 repeat 錯誤

### 2.1 基本修正流程
```
1. resolve_ws 必須返回 String（非 Option<String>）
2. 所有 workspace 參數必須是 Option<String>
3. 每次修改动動後，立即 run cargo check
4. 只有 check 通過後，才执行 cargo install
5. 安装後必測 2 個核心命令：
...
| `use of moved value: workspace` | 同一項在 loop 中多次调用 `resolve_ws()` | 在 loop 外解析一次，存入 `String` 後重用 |
| `TryFrom<&Option<String>>` | `Option<String>` 不能直接传给 HeaderValue | 改用 `.as_deref()` 或直接 `&resolved` |
...
# Step 2: Installation
cargo install --path . --force      # 安装到 ~/.cargo/bin/opendoc
...
| CLI 指令不可见 | binary 未安装 | `which opendoc && ls -l ~/.cargo/bin/opendoc` |
| 編譯報錯 | 類型不匹配 | `cargo check 2>&1 \| grep error` |

---

## 5. 注意事項
> - 禁止手動修改 `Cargo.lock` 或 `target/` 目录
> - `resolve_ws` 必须使用闭包形式，以避免引用捕获問題
> - 配置更新後，需要重新加载 `ConfigManager` 才能生效
> - 每改一次，必须立即测试，避免错误積累

---

> **更新時間**: 2026-07-27  
> **最后修订者**: AI 助手（遵循用戶要求，記錄核心修正常律）