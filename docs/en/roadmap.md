...
| **Candidate Technology — DuckDB FTS** | — | Not part of the current implementation and does not replace SQLite FTS5. Reconsider only if a very large corpus, batch analytics, or full-index rebuilds become a measured bottleneck and like-for-like benchmarks show gains that outweigh incremental-update, data-synchronization, binary-size, and cross-platform build costs. |
