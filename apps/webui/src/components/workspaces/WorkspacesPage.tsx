...
export function WorkspacesPage() {
  ...
  const refresh = async () => {
    ...
    setError(null);
    try {
      const [workspaceData, workbenchData] = await Promise.all([
        listWorkspaces(),
        getWorkbench(),
      ]);
      ...
      <div className="mx-auto max-w-6xl space-y-5">
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-[26px] font-semibold tracking-normal">
              {t("workspaces.title")}
            </h2>
            ...
            <section className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm">
                <p className="text-[22px] font-semibold">
                  {activeWorkspace?.name || t("common.unknown")}
                </p>
                <p className="mt-1 text-[12px] font-medium text-slate-500">
              ...
              <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm">
                <p className="text-[22px] font-semibold">
                  {workbench?.workspace.mode || t("common.unknown")}
                </p>
                <p className="mt-1 text-[12px] font-medium text-slate-500">
                  {t("workspaces.currentMode")}
                </p>
              </div>
            </section>
              ...
              <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-200 px-5 py-4">
                  <h3 className="text-[16px] font-semibold">
                    {t("workspaces.registry")}
                  </h3>
                  <p className="mt-1 text-[13px] text-slate-500">
                    {t("workspaces.registryDesc")}
                  </p>
                </div>
                {workspaces.length === 0 ? (
                  <div className="px-5 py-12 text-center text-[14px] text-slate-400">
                    {t("workspaces.empty")}
                  </div>
                ) : (
                        ...
                        >
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {isActive && (
                                <span className="rounded-md bg-blue-600 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white whitespace-nowrap">
                                  {t("common.active")}
                                </span>
                              )}
                            </div>
                ...
                <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[15px] font-semibold">
                      {t("workspaces.activeCorpus")}
                    </h3>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
                      <p className="text-[18px] font-semibold">
                        {workbench?.corpus.documents ?? 0}
                      </p>
